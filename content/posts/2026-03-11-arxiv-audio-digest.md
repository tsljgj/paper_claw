...
| 指标 | 数值 |
|:---|:---|
| ⏰ 时间窗口 | 2026-03-10 09:00:00 CST → 2026-03-11 09:00:00 CST |
| 📄 论文总数 | **10** 篇 |
...
> 💡 今日共收录 10 篇新论文，主要分布在 ASR 4 篇、Enhancement 2 篇、SLU 1 篇、Paralinguistics 1 篇、Audio 2 篇。
...
### 1. Speech-Omni-Lite: Portable Speech Interfaces for Vision-Language Models
...
| 项目 | 内容 |
|:---|:---|
| 📝 总结 | 这篇工作归入“语音识别”方向，核心任务由题目《Speech-Omni-Lite: Portable Speech Interfaces for Vision-Language Models》所界定。 从摘要看，作者主要围绕 asr 展开方法设计、训练策略或系统建模。 结果部分最值得注意的是：While large-scale omni-models have demonstrated impressive capabilities across various modalities, their strong performance heavily relies on massive multimodal data and incurs substantial computational costs. 如果你想快速判断这篇论文是否值得细读，这份摘要已经能帮助你抓住问题、方法和结果主线。 |
| 📖 可读性 | 可读性中。需要一定领域背景，但主线仍然清楚。 摘要更偏方法描述，建议点开原文确认实验细节。 优先看这些信号词：asr。 |

---
### 2. Finetuning a Text-to-Audio Model for Room Impulse Response Generation
...
| 项目 | 内容 |
|:---|:---|
| 📝 总结 | 这篇工作归入“语音识别”方向，核心任务由题目《Finetuning a Text-to-Audio Model for Room Impulse Response Generation》所界定。 从摘要看，作者主要围绕 asr、audio 展开方法设计、训练策略或系统建模。 结果部分最值得注意的是：In this paper, we propose a novel approach to RIR generation by fine-tuning a pre-trained text-to-audio model, demonstrating for the first time that large-scale generative audio priors can be effectively leveraged for the task. 如果你想快速判断这篇论文是否值得细读，这份摘要已经能帮助你抓住问题、方法和结果主线。 |
| 📖 可读性 | 可读性中。需要一定领域背景，但主线仍然清楚。 摘要更偏方法描述，建议点开原文确认实验细节。 优先看这些信号词：asr, audio。 |
...
| 项目 | 内容 |
|:---|:---|
| 📝 总结 | 这篇工作归入“语音识别”方向，核心任务由题目《A Semi-spontaneous Dutch Speech Dataset for Speech Enhancement and Speech Recognition》所界定。 从摘要看，作者主要围绕 asr、automatic speech recognition、speech recognition 展开方法设计、训练策略或系统建模。 结果部分最值得注意的是：DRES was designed as a test set for the evaluation of state-of-the-art (SOTA) automatic speech recognition (ASR) and speech enhancement (SE) models in a real-world scenario: a person speaking in a public indoor space with background talkers and noise. 如果你想快速判断这篇论文是否值得细读，这份摘要已经能帮助你抓住问题、方法和结果主线。 |
| 📖 可读性 | 可读性偏低。缩写、设定或实验细节较多，首次浏览成本偏高。 摘要中给出了明确指标，适合快速判断效果。 优先看这些信号词：asr, automatic speech recognition, speech recognition。 |

---
### 4. SCENEBench: An Audio Understanding Benchmark Grounded in Assistive and Industrial Use Cases
...
| 项目 | 内容 |
|:---|:---|
| 📝 总结 | 这篇工作归入“语音识别”方向，核心任务由题目《SCENEBench: An Audio Understanding Benchmark Grounded in Assistive and Industrial Use Cases》所界定。 从摘要看，作者主要围绕 audio language model、asr、automatic speech recognition 展开方法设计、训练策略或系统建模。 结果部分最值得注意的是：Advances in large language models (LLMs) have enabled significant capabilities in audio processing, resulting in state-of-the-art models now known as Large Audio Language Models (LALMs). 如果你想快速判断这篇论文是否值得细读，这份摘要已经能帮助你抓住问题、方法和结果主线。 |
| 📖 可读性 | 可读性偏低。缩写、设定或实验细节较多，首次浏览成本偏高。 摘要更偏方法描述，建议点开原文确认实验细节。 优先看这些信号词：audio language model, asr, automatic speech recognition。 |
...
### 1. A Fast Solver for Interpolating Stochastic Differential Equation Diffusion Models for Speech Restoration
...
| 项目 | 内容 |
|:---|:---|
| 📝 总结 | 这篇工作归入“语音增强”方向，核心任务由题目《A Fast Solver for Interpolating Stochastic Differential Equation Diffusion Models for Speech Restoration》所界定。 从摘要看，作者主要围绕 speech enhancement 展开方法设计、训练策略或系统建模。 结果部分最值得注意的是：One of the downsides of diffusion models is that solving the reverse process requires many evaluations of a large Neural Network. 如果你想快速判断这篇论文是否值得细读，这份摘要已经能帮助你抓住问题、方法和结果主线。 |
| 📖 可读性 | 可读性中。需要一定领域背景，但主线仍然清楚。 摘要更偏方法描述，建议点开原文确认实验细节。 优先看这些信号词：speech enhancement。 |

---
### 2. Distributed Multichannel Wiener Filtering for Wireless Acoustic Sensor Networks
...
| 项目 | 内容 |
|:---|:---|
| 📝 总结 | 这篇工作归入“语音增强”方向，核心任务由题目《Distributed Multichannel Wiener Filtering for Wireless Acoustic Sensor Networks》所界定。 从摘要看，作者主要围绕 speech enhancement、audio 展开方法设计、训练策略或系统建模。 结果部分最值得注意的是：We formally prove the optimality of dMWF and demonstrate its performance in simulated speech enhancement experiments. 如果你想快速判断这篇论文是否值得细读，这份摘要已经能帮助你抓住问题、方法和结果主线。 |
| 📖 可读性 | 可读性偏低。缩写、设定或实验细节较多，首次浏览成本偏高。 摘要更偏方法描述，建议点开原文确认实验细节。 优先看这些信号词：speech enhancement, audio。 |

---
## 🏷️ SLU

### 1. End-to-End Direction-Aware Keyword Spotting with Spatial Priors in Noisy Environments
...
| 项目 | 内容 |
|:---|:---|
| 📝 总结 | 这篇工作归入“口语理解”方向，核心任务由题目《End-to-End Direction-Aware Keyword Spotting with Spatial Priors in Noisy Environments》所界定。 从摘要看，作者主要围绕 keyword spotting 展开方法设计、训练策略或系统建模。 结果部分最值得注意的是：We present an end-to-end multi-channel KWS framework that exploits spatial cues to improve noise robustness. 如果你想快速判断这篇论文是否值得细读，这份摘要已经能帮助你抓住问题、方法和结果主线。 |
| 📖 可读性 | 可读性高。摘要结构较直白，问题、方法和结果都比较容易定位。 摘要更偏方法描述，建议点开原文确认实验细节。 优先看这些信号词：keyword spotting。 |

---
## 🏷️ Paralinguistics

### 1. EmoSURA: Towards Accurate Evaluation of Detailed and Long-Context Emotional Speech Captions
...
| 项目 | 内容 |
|:---|:---|
| 📝 总结 | 这篇工作归入“副语言学”方向，核心任务由题目《EmoSURA: Towards Accurate Evaluation of Detailed and Long-Context Emotional Speech Captions》所界定。 从摘要看，作者主要围绕 emotion、audio 展开方法设计、训练策略或系统建模。 结果部分最值得注意的是：Our experiments show that EmoSURA achieves a positive correlation with human judgments, offering a more reliable assessment for long-form captions compared to traditional metrics, which demonstrated negative correlations due to their sensitivity to caption length. 如果你想快速判断这篇论文是否值得细读，这份摘要已经能帮助你抓住问题、方法和结果主线。 |
| 📖 可读性 | 可读性中。需要一定领域背景，但主线仍然清楚。 摘要更偏方法描述，建议点开原文确认实验细节。 优先看这些信号词：emotion, audio。 |
...
| 项目 | 内容 |
|:---|:---|
| 📝 总结 | 这篇工作归入“通用音频”方向，核心任务由题目《Physics-Informed Neural Engine Sound Modeling with Differentiable Pulse-Train Synthesis》所界定。 从摘要看，作者主要围绕 audio 展开方法设计、训练策略或系统建模。 结果部分最值得注意的是：Validated on three diverse engine types totaling 7.5 hours of audio, PTR achieves a 21% improvement in harmonic reconstruction and a 5.7% reduction in total loss over a harmonic-plus-noise baseline model, while providing interpretable parameters corresponding to physical phenomena. 如果你想快速判断这篇论文是否值得细读，这份摘要已经能帮助你抓住问题、方法和结果主线。 |
| 📖 可读性 | 可读性中。需要一定领域背景，但主线仍然清楚。 摘要中给出了明确指标，适合快速判断效果。 优先看这些信号词：audio。 |

---
### 2. MUGEN: Evaluating and Improving Multi-audio Understanding of Large Audio-Language Models
...
| 项目 | 内容 |
|:---|:---|
| 📝 总结 | 这篇工作归入“通用音频”方向，核心任务由题目《MUGEN: Evaluating and Improving Multi-audio Understanding of Large Audio-Language Models》所界定。 从摘要看，作者主要围绕 audio、music 展开方法设计、训练策略或系统建模。 结果部分最值得注意的是：Combining this permutation strategy with Chain-of-Thought further improves performance to 6.74%. 如果你想快速判断这篇论文是否值得细读，这份摘要已经能帮助你抓住问题、方法和结果主线。 |
| 📖 可读性 | 可读性高。摘要结构较直白，问题、方法和结果都比较容易定位。 摘要中给出了明确指标，适合快速判断效果。 优先看这些信号词：audio, music。 |
